...
data  = read.csv(paste0(data_path,"facebook/Training/Features_Variant_2.csv"), header = F)
data = scale(data)
...
for(i in 1:iterations){
  ...
  comparison_rets[[i]] = LCPcompare(xtrain = xtrain, ytrain = ytrain, xcalibration = xcalibration,
                                    ...
                                    save_path = paste0('UCI_results/facebook2/'), print_out = 10)
  print( comparison_rets[[i]]$lens)
  saveRDS(comparison_rets, file = paste0("UCI_results/facebook2_LCPcomparison_results.rds"))
}


comparison_rets=readRDS("UCI_results/facebook2_LCPcomparison_results.rds")
